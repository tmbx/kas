...
static void rf_host_tight_decompress_dispatcher(void);
...
static void rf_host_tight_jpeg(void);
...
static void rf_host_tight_compctl(void)
{
  ...
  s_compression_id = comp_ctl & 0xF0;             /* clear bits 3..0 */

  if (s_compression_id == RFB_TIGHT_FILL) {
    aio_setread(rf_host_tight_fill, NULL, 3);
  }
  else if (s_compression_id == RFB_TIGHT_JPEG) {
    s_uncompressed_size = s_rect.w * s_rect.h * 3;
    aio_setread(rf_host_tight_len1, NULL, 1);
  }
  else if (s_compression_id > RFB_TIGHT_MAX_SUBENCODING) {
    log_write(LL_ERROR, "Invalid sub-encoding in Tight-encoded data");
    aio_close(0);
    return;
  }
  else {                        /* "basic" compression */
    s_stream_id = (s_compression_id >> 4) & 0x03;
    if (s_compression_id & RFB_TIGHT_EXPLICIT_FILTER) {
...
static void rf_host_tight_len1(void)
{
  ...
  if (cur_slot->readbuf[0] & 0x80) {
    ...
    rf_host_tight_decompress_dispatcher();
  }
}
...
static void rf_host_tight_decompress_dispatcher(void)
{
  if (s_compression_id == RFB_TIGHT_JPEG)
    aio_setread(rf_host_tight_jpeg, NULL, s_compressed_size);
  else
...
{
  cinfo->src->next_input_byte = cur_slot->readbuf;
  cinfo->src->bytes_in_buffer = s_compressed_size;
}

static boolean jpeg_mem_fill_input_buffer(j_decompress_ptr cinfo)
{
  log_write(LL_INFO, "%s called ??? This should not happen.", __func__ );
  log_write(LL_ERROR, "Jpeg decompression error, incomplete jpeg received.");
  /* Insert a fake EOI marker */
  cur_slot->readbuf[0] = (JOCTET) 0xFF;
  cur_slot->readbuf[1] = (JOCTET) (JOCTET) JPEG_EOI;
  cinfo->src->next_input_byte = cur_slot->readbuf;
...
{
  log_write(LL_INFO, "%s called.", __func__ );
...
{
  struct jpeg_source_mgr *jsrc = (struct jpeg_source_mgr *)(*cinfo->mem->alloc_small)(cinfo, JPOOL_PERMANENT, sizeof(struct jpeg_source_mgr));
  jsrc->next_input_byte = NULL;
  jsrc->bytes_in_buffer = 0;
  jsrc->init_source = jpeg_mem_init_source;
  jsrc->fill_input_buffer = jpeg_mem_fill_input_buffer;
  jsrc->skip_input_data = jpeg_mem_skip_input_data;
  jsrc->resync_to_restart = jpeg_resync_to_restart;
  jsrc->term_source = jpeg_mem_term_source;

  cinfo->src = jsrc;
}

struct vnc_jpeg_error
{
...
{
  struct vnc_jpeg_error *err = (struct vnc_jpeg_error *)cinfo->err;
...
static void jpeg_output_message(j_common_ptr cinfo)
{
  ...
  log_write(LL_ERROR, "libjpeg: %s", buffer);
}

static void rf_host_tight_jpeg(void)
{
  struct jpeg_decompress_struct cinfo;
  struct vnc_jpeg_error jerr;
  CARD8 *buf, *buffer;
  JSAMPARRAY *line;

  log_write(LL_INFO, "Receiving a JPEG encoded rectangle.");
  ...
  buf = buffer = (CARD8 *)malloc(cinfo.output_width * cinfo.output_components * cinfo.output_height * sizeof(CARD8));

  while (cinfo.output_scanline < cinfo.output_height) {
    jpeg_read_scanlines(&cinfo, &buf, 1);
    buf += cinfo.output_width * cinfo.output_components;
  }
      
  tight_draw_truecolor_data(buffer);
