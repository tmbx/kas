...
static void rf_host_set_formats(void)
{
  ...
  if (s_request_tight) {
    ...
    if (s_tight_quality >= 0 && s_tight_quality <= 9) {
      log_write(LL_DETAIL, "Requesting jpeg level %d", s_tight_level);
      buf_putsafe_CARD32(&setenc_msg[4 + num_enc++ * 4],
                         RFB_ENCODING_QUALITYLEVEL0 + (CARD32)s_tight_quality);
    }
  }
