...
"HorizonLive, please see our website at http://www.horizonlive.com/\n\n",
  ...
  if (init_screen_info()) {
    ...
    set_host_encodings(opt_request_copyrect, opt_convert_copyrect,
                       opt_request_tight, opt_tight_level, opt_tight_quality, opt_request_cursor);
    ...
    do {
      if (kas_opt.used) {
        log_write(LL_MSG, "Connected to KAS host");
        aio_add_slot(kas_opt.host_fd, NULL, host_init_hook, sizeof(HOST_SLOT));
      }
	
      else {
        if (! connect_to_host(opt_host_info_file, opt_cl_listen_port)) break;
      }
	
      if (! write_pid_file()) break;
...
static void parse_args(int argc, char **argv)
{
  ...
  while (!err &&
         (c = getopt(argc, argv, "k:hqjrRxv:f:p:a:c:g:l:i:s:b:tT:Q:")) != -1) {
    switch (c) {
    case 'k':
    {
      char *s = strstr(optarg, ":");
      if (s == NULL) { err = 1; break; }
      *s = 0;
      kas_opt.used = 1;
      kas_opt.host_fd = atoi(optarg);
      kas_opt.client_fd = atoi(s + 1);
      break;
    }
    case 'h':
      err = 1;
    ...
    case 'T':
      ...
      break;
    case 'Q':
      if (opt_tight_quality >= 0)
          err = 1;
      else {
          opt_request_tight = 1;
          opt_tight_quality = atoi(optarg);
          if (opt_tight_quality < -1 || 9 < opt_tight_quality)
              err = 1;
      }
      break;
    default:
  ...
  if (opt_log_filename == NULL)
    opt_log_filename = "/var/log/reflector.log";
  if (opt_cl_listen_port == -1)
...
static void report_usage(char *program_name)
{
  ...
  fprintf(stderr,
          "Options:\n"
	  "  -k host_fd ':'  - Do KAS-style host connection.\n"
	  "     client_fd\n"
  ...
  fprintf(stderr,
          ...
          " level (1..9)\n"
          "  -Q              - like -T, but specify the jpeg quality (0..9)."
          " use in conjonction with -T.\n."
