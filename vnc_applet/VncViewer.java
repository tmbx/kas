...
public class VncViewer extends java.applet.Applet
  ...
  // Helper for communicating with javascript - also implements the KDebug interface
  public JSCommVNC jscomm = null;
  ...
  // Teambox parameters
  int debug_level;
  public String js_debug_func = null;
  public String js_status_func = null;  
  public boolean teambox_mode;
  public String teambox_auth;
  public String teambox_cert;
  ...
  public void init() {
    ...
    // Part one... bugs here won't get to the browser.
    try {
        // Helper for communicating with javascript - implements the KDebug interface
        jscomm = new JSCommVNC(this);

        // Get parameters.
        readParameters();
    } catch (KException e) {
        // Part one failed with a user exception.
    ...
    // Part two... from now on, status is sent to browser.
    try {
        jscomm.debug(1, "vnc_viewer", "Screen sharing applet loaded.");
        jscomm.status(JSCommVNC.VNC_STATUS_APPLET_LOADED);

        if (inSeparateFrame) {
          vncFrame = new Frame("Teambox Screen Sharing");
          if (!inAnApplet) {
    ...
    {
        jscomm.debug(0, "vnc_viewer", "init() exception: '" + e.getMessage() + "'");
        jscomm.status(JSCommVNC.VNC_STATUS_FAILED);
        e.printStackTrace();
    }
  }
  ...
  public void hideStatus() {
    vncContainer.remove(statusLabel);
  }
        ...
        throws Exception
  {
        ...
        if (t != null && !t.trim().equals("")) { debug_level = Integer.parseInt(t.trim()); }
        else { debug_level = 1; }
        jscomm.set_debug_level(debug_level);
        jscomm.debug(4, "vnc_viewer", "Debug level: " + debug_level);

        // Javascript debug function (facultative)
        t = getParameter("js_debug_func");
        if (t != null && !t.trim().equals("")) 
        { 
            js_debug_func = t.trim();
            jscomm.set_js_debug_func(js_debug_func);
        }
        //else { throw new KException("Missing javascript debug function parameter."); }
        jscomm.debug(4, "vnc_viewer", "Javascript debug function: " + js_debug_func);

        // Javascript status function (mandatory)
        t = getParameter("js_status_func");
        if (t != null && !t.trim().equals("")) 
        { 
            js_status_func = t.trim(); 
            jscomm.set_js_status_func(js_status_func);
        }
        else { throw new KException("Missing javascript status function parameter."); }
        jscomm.debug(4, "vnc_viewer", "Javascript status function: " + js_status_func);

        // Teambox mode (facultative)
        t = getParameter("teambox_mode");
        teambox_mode = false;
        if (t != null && t.trim().equals("1")) { teambox_mode = true; }
        jscomm.debug(4, "vnc_viewer", "Teambox mode: " + teambox_mode);
        
        if (teambox_mode == true)
        {
            // Teambox authentication string (mandatory)
            t = getParameter("teambox_auth");
            if (t != null && !t.trim().equals("")) { teambox_auth = t.trim(); }
            else { throw new KException("Teambox authentication string is needed in teambox mode."); }
            jscomm.debug(4, "vnc_viewer", "Teambox authentivation string: " + teambox_auth);

            // KCD cert (mandatory)
            t = getParameter("teambox_cert");
            if (t != null && !t.trim().equals("")) { teambox_cert = t.replace("%", "\n"); }
            else { throw new KException("Teambox cert is needed in Teambox mode."); }
            jscomm.debug(4, "vnc_viewer", "Teambox certificate has " + teambox_cert.length() + " bytes.");
            jscomm.debug(4, "vnc_viewer", "Teambox certificate is: " + teambox_cert);
        }
  ...
  synchronized public void fatalError(String str) {
    ...
    jscomm.debug(0, "vnc_viewer", "Fatal error: " + str);
    jscomm.status(JSCommVNC.VNC_STATUS_FAILED, str);
    ...
    if (inAnApplet) {
      if (teambox_mode) {
          if (e instanceof AccessControlException) {
            jscomm.debug(1, "vnc_viewer", "Trust has been denied.");
            jscomm.status(JSCommVNC.VNC_STATUS_FAILED, "user did not authorize applet to load"); 
          } else if (e instanceof EOFException) {
            jscomm.debug(1, "vnc_viewer", "Remote side closed the connection.");
            jscomm.status(JSCommVNC.VNC_STATUS_CLOSED);
          } else if (e instanceof NoRouteToHostException) {
            jscomm.debug(1, "vnc_viewer", "Failed: " + e.getMessage());
            jscomm.status(JSCommVNC.VNC_STATUS_FAILED, "no route to server");
          } else if (e instanceof UnknownHostException) {
            jscomm.debug(1, "vnc_viewer", "Failed: " + e.getMessage());
            jscomm.status(JSCommVNC.VNC_STATUS_FAILED, "unknown host");
          } else if (e instanceof ConnectException) { 
            jscomm.debug(1, "vnc_viewer", "Failed: " + e.getMessage());
            jscomm.status(JSCommVNC.VNC_STATUS_FAILED, "could not connect to host"); 
          } else { 
            jscomm.debug(1, "vnc_viewer", "Failed: class: " + e.getClass().toString() + " exception: " + e.getMessage());
            jscomm.status(JSCommVNC.VNC_STATUS_FAILED);
          }
      } else {
        showMessage(str);
