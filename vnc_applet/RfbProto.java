...
class RfbProto {
  ...
  // Teambox mode
  boolean teambox_mode = false;

  // Teambox authentication string
  String teambox_auth = null;

  // Teambox certificate
  String teambox_cert = null;

  // Helper for communicating with javascript - also implements the KDebug interface
  private JSCommVNC jscomm = null;

  // Helper for communicating with the KCD
  KasConn kasconn = null;

  //
  // Constructor. Make TCP connection to RFB server.
  //

  RfbProto(String h, int p, VncViewer v) throws Exception {
    viewer = v;
    host = h;
    port = p;

    teambox_mode = viewer.teambox_mode;
    teambox_auth = viewer.teambox_auth;
    teambox_cert = viewer.teambox_cert;
    jscomm = viewer.jscomm;

    if (viewer.socketFactory == null) {
      if (teambox_mode == false) {
        ...
        try {
          ...
          // Connecting...
          jscomm.debug(2, "vnc_viewer", "Connecting...");
          jscomm.debug(4, "vnc_viewer", "Connecting to vnc meta proxy...");
          jscomm.status(JSCommVNC.VNC_STATUS_CONNECTING);
          kasconn = new KasConn(jscomm);
          kasconn.setSSLCertsString(teambox_cert);
          kasconn.dispatchConnect(h, p);
          jscomm.debug(4, "vnc_viewer", "Connected to vnc meta proxy.");
          sock = kasconn.getSSLSocket();

          // Getting redirected by VNC meta proxy.
          jscomm.debug(5, "vnc_viewer", "Getting a VNC redirection on the vnc meta proxy..."); 
          os = sock.getOutputStream();
          os.write("VNC!".getBytes());
          os.write(teambox_auth.getBytes("UTF8"));
          os.flush();
          jscomm.debug(4, "vnc_viewer", "Proxied by vnc meta proxy.");

          // Hiding the "Connecting..." label in the java applet.
          viewer.hideStatus();

          // Connected...
          jscomm.debug(1, "vnc_viewer", "Connected to remote screen.");
          jscomm.status(JSCommVNC.VNC_STATUS_CONNECTED);
        } catch(Exception e) {
          // Connection failed.
          jscomm.debug(0, "vnc_viewer", "Could not connect: " + e.getMessage());
          e.printStackTrace();
          throw e;
        }
      } 
  ...
  synchronized void close() {
    try {
      if (teambox_mode == true) {
        jscomm.debug(2, "vnc_viewer", "Closing connection...");
        kasconn.close();
        jscomm.debug(1, "vnc_viewer", "Closed connection.");
